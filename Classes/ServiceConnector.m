...
-(void)postLocations:(NSArray *)dbLocations{
    ...
    NSString *dcsLocationAPI = @"location_update";
    NSString *dcsLocationUpdatePushUrl = [NSString stringWithFormat:@"%@/%@/", DCSUrl, dcsLocationAPI];

    NSLog(@"Sending locations to %@: ", dcsLocationUpdatePushUrl);

    //build up request url
    NSMutableURLRequest *request = [[NSMutableURLRequest alloc] initWithURL:
                                    [NSURL URLWithString: ([DCSUrl length] == 0) ? @"http://cycl-ops.se.rit.edu/location_update/" : dcsLocationUpdatePushUrl] ];//must update
    //add Method
